/**
 * Purpose: filesystem and remote-clone trust-boundary helpers.
 * Responsibilities: validate clone hosts, branches, repository paths, symlinks,
 * writable targets, and bounded recursive listings.
 * Constraints: callers must validate before touching a clone; paths must remain
 * inside the clone's real root and `.git`/option-like segments remain forbidden.
 * Relationships: server.js uses these helpers for every clone file/tree endpoint.
 */
const fs = require('fs');
