/**
 * Purpose: optional hosted GitHub App authentication and repository proxy layer.
 * Responsibilities: OAuth state/session lifecycle, installation-scoped repository
 * checks, safe GitHub content access, and review-PR batch publishing.
 * Constraints: tokens stay server-side and in bounded process memory; every request
 * is installation-authorized and path/branch validated; this module is not exposed
 * by the static Pages or local-only desktop edition.
 */
...
function registerGitHubRoutes(app, options = {}) {
  ...
  const callbackHandler = route(async (req, res) => {
  ...
  app.get(['/auth/github/callback', '/api/auth/github/callback'], callbackHandler);
