...
function createPostgresJobStore(pool) {
  ...
  return {
    ...
    ,async retry(id) { const current = await this.get(id); if (!current) return null; const result = await pool.query(`UPDATE build_jobs SET status='queued', updated_at=now(), error=NULL WHERE id=$1 AND status='failed' RETURNING *`, [id]); return mapRow(result.rows[0] || current); }
  };
}
